...
class window.Gradient extends Control
  ...
  _redraw: ->
    ...
    if direction and start and end
      ...
      # TODO: Use new linear-gradient syntax for browsers that support it.
      value = if Control.browser.mozilla
        property = "background-image"
        position = if horizontal then "left" else "top"
        "-moz-linear-gradient( #{position}, #{startColorString}, #{endColorString} )"
      else if Control.browser.webkit
        property = "background-image"
        position2 = if horizontal then "right top" else "left bottom"
        "-webkit-gradient( linear, left top, #{position2}, from( #{startColorString} ), to( #{endColorString} ) )"
      else if Control.browser.msie
        property = "filter"
        gradientType = if horizontal then 1 else 0
        "progid:DXImageTransform.Microsoft.gradient( gradientType=#{gradientType}, startColorStr=#{startColorString}, endColorStr=#{endColorString} )"
      @css property, value

  # Convert a hex color like #00ff00 to "rgb(0, 255, 0)" 
  _hexColorToRgbString: ( hex ) ->
    if hex.substr( 0, 1 ) is "#"
      hex = hex.substring( 1 ) # Remove "#"
    hasAlpha = ( hex.length is 8 )
    color = parseInt hex, 16
    if Control.browser.msie  # Internet Explorer
      rgbString = hex
      if hasAlpha
      ...
      g = ( color >> 8 ) & 0xFF
      b = color & 0xFF
      "#{colorStringType}(#{r},#{g},#{b}#{alphaString})"
